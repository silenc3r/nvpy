...
class RedirectedText(tk.Text):
    ...
    def cursor_pos(self) -> Tuple[int, int]:
        """Get position of the cursor. Represented as (line, column)."""
        line, col = self.index("insert").split(".")
        return (int(line), int(col))

    def get_current_line(self) -> str:
        """Get current line text. Newline char not included."""
        start, end = self.index("insert linestart"), self.index("insert lineend")
...
class NotesList(tk.Frame):
    ...
    def __init__(self, master, font_family, font_size, config: NotesListConfig):
        ...
        self.text.tag_config(
            "found", font=italic_font, foreground=config.colors.note_info, background=config.colors.highlight_note_info
        )
    ...
    def append(self, note, config):
        ...
        if self.layout == "vertical" and self.print_columns == 1:
            ...
            if tags and not self.hide_tags:
                if config.tagfound:
                    self.text.insert(
                        tk.END, "{0:<{w}}".format(",".join(tags)[: cellwidth - 1], w=cellwidth), ("found",)
                    )
                else:
...
class NoteText(RedirectedText):
    """
    NoteText is tk.Text widget with ability to complete note title when
    completion mark "[[" is found to the left of the cursor position.

    Keybindings:
        Ctrl-space: start completion or select next candidate
        Return or Right arrow: confirm completion
    """
    ...
    def __init__(self, master, case_sensitive: int, **kwargs):
        ...
        self.bind("<Button-1>", lambda _: self._end_complete())
    ...
    def triggeredcomplete(self) -> None:
        ...
        if self.cycle:
            ...
            word_end = cur_col
            close_bracket_pos = current_line.find("]]", word_start)
            if close_bracket_pos != -1:
                # when closing brackets are found include them in word_end
                # but not in word_prefix_end
            ...
            self.cursor_index = "%d.%d" % (line, word_prefix_end)
            self.word_start_index = "%d.%d" % (line, word_start)
            self.word_end_index = "%d.%d" % (line, word_end)

            # collect hits
            if word_start == word_prefix_end:
                # prefix is emty string
                hits = self._completion_list[:]
            else:
                hits = []
                word_prefix = current_line[word_start:word_prefix_end]
                word_prefix = word_prefix if self.case_sensitive else word_prefix.lower()
                for element in self._completion_list:
                    el = element if self.case_sensitive else element.lower()
                    if el.startswith(word_prefix):
                        hits.append(element)
        ...
        if self._hits:
            ...
            new_eow_index = "%d.%d" % (line, word_start + len(new_word))

            self.delete(self.word_start_index, self.word_end_index)
            self.insert(self.word_start_index, new_word)
...
class View(utils.SubjectMixin):
    ...
    def cmd_root_delete(self, evt=None):
        ...
        if self.config.confirm_delete:
            ...
            if tkMessageBox.askyesno(
                "Really delete note?", "Are you sure you want to delete the current note?", default=tkMessageBox.NO
            ):
                is_delete = True
        else:
    ...
    def set_note_tally(self, filtered_notes, active_notes, total_notes):
        self.statusbar.set_centre_status(
            "Listing %d / %d active notes (%d total)" % (filtered_notes, active_notes, total_notes)
        )
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(
            label="Sync current note", underline=0, command=self.cmd_sync_current_note, accelerator="Ctrl+S"
        )
        ...
        file_menu.add_command(
            label="Render Markdown to HTML", underline=7, command=self.cmd_markdown, accelerator="Ctrl+M"
        )
        ...
        edit_menu.add_command(
            label="Find", accelerator="Ctrl+F", underline=0, command=lambda: self.search_entry.focus()
        )
    ...
    def _create_ui(self):
        ...
        self.search_entry = TriggeredcompleteEntry(
            search_frame, self.config.case_sensitive, textvariable=self.search_entry_var, style="Search.entry"
        )
        ...
        notes_list_config = NotesListConfig(
        ...
        if self.config.layout == "horizontal":
            ...
            self.notes_list = NotesList(
                list_frame, self.config.list_font_family, self.config.list_font_size, notes_list_config
            )
    ...
    def handler_control_delete(self, evt):
        ...
        if insert in "  " and insert != "":
            ...
            if insert in "\n\t" and insert != "":
                if self.text_note.get(tk.INSERT, tk.END).strip() == "" and self.text_note.index(
                    tk.INSERT
                ) != self.text_note.index(tk.END):
