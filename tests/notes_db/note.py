...
class NoteOperations(DBMixin, unittest.TestCase):
    ...
    def test_get_status(self):
        ...
        self.assertEqual(
            db.get_note_status("MODIFIED"), NoteStatus(saved=False, synced=False, modified=True, full_syncing=False)
        )
        self.assertEqual(
            db.get_note_status("SAVED"), NoteStatus(saved=True, synced=False, modified=False, full_syncing=False)
        )
        self.assertEqual(
            db.get_note_status("SYNCED"), NoteStatus(saved=True, synced=True, modified=False, full_syncing=False)
        )
        # todo: NoteStatus.modified = not NoteStatus.saved. NoteStatus.modified can be replace to the property.
        self.assertEqual(
            db.get_note_status("SYNCED_BUT_NOT_SAVED"),
